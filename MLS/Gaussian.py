...
class NaiveBayesClassifier:
    ...
    def predict_proba(self,X_t):
        ...
        probs=np.zeros((X_t.shape[0],len(self.classes)))
        ...
        for clas in self.classes:
            for i in range(0,X_t.shape[0]):
                ...
                for col in X_t.columns:
                    if pd.api.types.is_numeric_dtype(X_t[col]):
                        log_t+=np.log((self.normal_likelihood(X_t.loc[i,col],self.numerical[clas][col][0],
                                                    self.numerical[clas][col][1])))
                    else:
                        log_t+=np.log(self.categorical[clas][col].get(X_t.loc[i, col], 1e-9))
